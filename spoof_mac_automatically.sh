...
plugin_enabled=1
if [[ -z "${AIRGEDDON_DO_NOT_SPOOF_MAC_ADDRESS_AUTOMATIC}" ]]; then
    AIRGEDDON_DO_NOT_SPOOF_MAC_ADDRESS_AUTOMATIC=0
fi
...
function spoof_mac_automatically_override_et_prerequisites() {
    ...
    if [ ${AIRGEDDON_DO_NOT_SPOOF_MAC_ADDRESS_AUTOMATIC} -eq 1 ]; then
	    ask_yesno 419 "no"
	    if [ "${yesno}" = "y" ]; then
